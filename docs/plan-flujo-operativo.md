...
**Triage, estabilidad y prioridad (Fase 4 / [ADR-003](./fase-0/adr-contingencias.md)):** el contrato expone `droppedDetails` (los puntos que quedarían sin atender, con `fillPct`, `criticality` y `priority`, **ordenados por criticidad** según el modelo único de ADR-002) y KPIs numéricos por paso (`reassignedPoints`, `before/afterDistanceKm`, `distanceDeltaKm`, `baseStops`, `stabilityPct`). `beforeDistanceKm` y `afterDistanceKm` describen el **día completo** (antes = rutas vigentes; después = antes − ruta averiada + recálculo), así que el delta es comparable; el aviso usa la **flota del día**, no los vehículos `available` de la BD. La UI de la simulación muestra **“Estabilidad del plan”** y el triage; y ambos recálculos de contingencia activan `priority_fill_level=True`, de modo que el ACO **favorece contenedores críticos** (≥ 80 %) y en riesgo cuando la flota es justa.
...
- **Máquina de estados del día:** `draft → optimized → dispatched → closed/partial` (`DailyPlan.status`), visible como **chip** en la cabecera del día y como **leyenda** en el calendario semanal. **Cerrar día** aparece en la barra al despachar (único punto de acceso: ni en ⋯ ni en el estado vacío de Resultados). Los **resultados reales** (previsto vs. real) se muestran solo en la pestaña **Resultados** cuando el día está cerrado (`partial`/`closed`); el previsto vive en el tab **Plan**.
- **Banda de estado contextual (BDC):** una sola banda sobre los tabs, resuelta por `resolveOptimizationContextBand` con prioridad **error > semana sin aprobar > despacho > cierre** (`OptimizationContextBand`). El escenario heredado se muestra como **chip** en la cabecera (`optimization-scenario-chip`, P1) y los pendientes como conteo en la pestaña **Pendientes**; ya no hay tarjeta permanente «Situación del día».
