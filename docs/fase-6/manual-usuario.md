...
2. Pestaña **Plan**:
   ...
   2. Pulse **Generar rutas del día** (optimización ACO real, sin envío a campo). Para **simular la jornada**, use **Simular día** (botón en el tab **Rutas por vehículo**, botón en el estado vacío de **Resultados**, enlace en Resultados → bloque «Previsto», o ⋯ → Simular día…): elija el **escenario del día** (si difiere del plan, la simulación **recalcula la jornada** bajo ese escenario — p. ej. «Lluvia intensa»), si hay **condiciones extraordinarias** (averías/contenedor crítico) y cómo verlo — **Ver la animación** (abre el recorrido del día y se detiene en cada contingencia) o **Calcular en 2º plano** (solo el resumen de resultados, sin animación). El resto de acciones secundarias (**Simular contingencia**, **Reenviar notificación**, **Exportar PDF**, **Ver en mapa operativo**) viven en el menú **⋯** (Herramientas).
   3. Revise el **Resumen del día** (tarjeta sobre el mapa: puntos programados, contenedores críticos, distancia, duración de flota, jornada por camión, toneladas y ahorro vs. línea base). El detalle por camión está en la pestaña **Rutas por vehículo** (tabla con total, junto a **Simular día**). La comparativa **línea base del turno vs. plan** y el **desglose de duración** aparecen en la pestaña **Resultados** (bloque «Previsto») **después de ejecutar Simular día** para esa fecha; hasta entonces la pestaña muestra un aviso unificado con el botón **Simular día**. Ya simulada la jornada pero sin cerrar, el bloque **Real** queda como nota compacta («La simulación del día ya se calculó; el previsto vs. real aparecerá al cerrar el día»).
   ...
   6. Al finalizar la jornada, use **Cerrar día** (único punto: botón visible en la barra cuando el día está **despachado**; no se duplica en ⋯ ni en el estado vacío de Resultados).
   7. Sin datos de campo, el **real** del cierre queda vacío (0 puntos servidos, sin distancia/duración/peso). Para demostrar el ciclo **previsto → real**, use **⋯ → Simular ejecución del día** (o el botón en **Resultados → Real** cuando el comparativo está a 0): registra una **ejecución simulada** (paradas visitadas con llegada estimada como real) y consolida el comparativo. Después, **Cerrar día** fija el resultado. No se puede simular la ejecución de un día ya cerrado.
3. Pestaña **Resultados** (solo con el día **cerrado**): **previsto vs. real** del día (distancia, duración, puntos servidos y cumplimiento). Si el día está despachado pero sin cerrar muestra «Jornada en curso»; si aún no hay cierre, «Aún no hay resultados reales». Si el cierre no tuvo ejecución registrada, el panel explica que no hay datos reales (ninguna parada se confirmó).
