...
export function SelectField(props: SelectFieldProps) {
  ...
  return (
    <div class={local.class}>
      ...
      <select
        id={selectId()}
        class={`w-full appearance-none rounded-md border border-default bg-elevated bg-[length:1rem] bg-[right_0.75rem_center] bg-no-repeat py-3 pl-4 pr-10 text-sm text-text-primary transition-colors focus:border-fero-blue focus:outline-none focus:ring-2 focus:ring-fero-blue/20 ${local.selectClass ?? ''}`}
        style={{
          'background-image':
