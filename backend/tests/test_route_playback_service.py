...
from app.services.route_playback_service import (
    build_daily_route_playback,
    build_simulation_route_playback,
)
...
def test_build_daily_route_playback_includes_stop_type_on_all_stops():
    db = MagicMock()
    plan = SimpleNamespace(
        id=7,
        operation_date=date(2026, 8, 14),
    ...
    for stop in payload["routes"][0]["stops"]:
        assert stop["stopType"] in {"collection", "landfill"}
...
def test_build_simulation_route_playback_returns_routes():
    db = MagicMock()
    simulation = SimpleNamespace(
        id=55,
        executed_at=None,
        parameters_json='{"operatorsShortage": 0}',
    )
    route = optimized_route(
        31,
        status="pending",
    ...
    route.simulation_id = 55
    route.estimated_duration_seconds = 2400
    route.vehicle.id = 2

    db.get.side_effect = lambda model, pk: simulation if pk == 55 else None
    db.scalars.return_value.unique.return_value.all.return_value = [route]

    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(
            route_playback_service,
        ...
        payload = build_simulation_route_playback(db, 55)

    assert payload["simulationId"] == 55
    assert payload["previewMode"] is True
    assert len(payload["routes"]) == 1
    assert payload["routes"][0]["stops"][0]["stopType"] == "collection"
