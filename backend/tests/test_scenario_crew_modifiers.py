...
def test_shortage_two_on_full_crew_yields_twenty_six_minutes_per_stop():
    ...
    assert modifiers["baselineServiceSecondsPerStop"] == BASE_SERVICE_SECONDS == 1200


def test_compute_service_time_shortage_zero_vs_two():
    vehicle = _vehicle(6)
    assert compute_service_time_sec(vehicle, operators_shortage=0) == 1200
    assert compute_service_time_sec(vehicle, operators_shortage=2) == 1560
...
def test_resolve_fleet_crew_applies_shortage_before_engine():
    ...
    assert compute_service_time_sec(resolved[0]) == 1560
